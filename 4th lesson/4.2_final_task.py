...
-- ПРИНЦИП РАБОТЫ --
Хеш-таблица реализована на массиве. Коллизии разрешаются метода цепочек, на основе связанных список. По условиям задачи
одновременно может быть записано < = 10^5 ключей, хорошим размером для таблицы будет простое число, примерно равное
10^5 * 4/3 (для достижения наилучшей работы хеш-таблицы нужно иметь запас пустых корзин. Опытным путём выяснили, что
хеш-таблицы работают наиболее эффективно, когда они заполнены не больше, чем на три четверти.) Принцип работы:
на полученный ключ находим индекс, куда класть/доставать/удалять. При возникновении коллизии, мы даем текущей паре
(ключ, значение) ссылку на следующую и при получении/добавлении/удалении проверяем вариант коллизии.
